...
import socket, time, re, socket, ethtool, hashlib, logging, logging.handlers, os, os.path, json, ipaddr, atexit

try: import dnet
except ImportError: import dumbnet as dnet
...
def addr2daddr(addr, maskbits):
	z = []
	for i in xrange(1, 5): z.append(str(int(addr[-2*i]+addr[+1+-2*i], 16)))
	return(dnet.addr(".".join(z)+"/"+str(maskbits)))
def mask2bitnum(mask):
	z = ''
	for i in xrange(1, 5): z += mask[-2*i]+mask[+1+-2*i]
	n = bin(int(z, 16)).lstrip("0b")
	if len(n) != 32: return(32)
	n = n.rstrip('0')
	if len(n.rstrip('1')) != 0: return(32)
	else: return(len(n))
...
def get_routes():
	...
	#logger.debug("Return %d routes : %s" % (len(r), str(r)))
	return r

def get_lapis_route(routes, gateway=None):
	if gateway: gateway = dnet.addr(str(gateway))
	for r in list(routes):
		if r[0] != MCAST_LAPIS_IFACE:
			routes.remove(r)
			#logger.debug("Removed non nsrpd route : %s", r)
		elif gateway and r[2] != gateway:
			routes.remove(r)
			#logger.debug("Removed non selected route : %s", r)
	#logger.debug("Return %d nsrpd route selected by %s : %s" % (len(routes), str(gateway), routes))
	return(routes)

def get_local_routes(routes):
	for r in list(routes):
		if r[0] == MCAST_LAPIS_IFACE:
			#logger.debug("Removed nsrpd route %s " % str(r))
			routes.remove(r)
		elif r[0] in IGNORE_IFACE:
			#logger.debug("Removed exclued interface %s " % str(r))
			routes.remove(r)
		elif str(r[2]) != "0.0.0.0": 
			#logger.debug("Removed distant route %s " % str(r))
			routes.remove(r)
	#logger.debug("Return %d local routes : %s" % (len(routes), str(routes)))
	return routes

def agregate_network(networks):
	rsort = []
	for net in networks: rsort.append(ipaddr.IPv4Network(str(net)))
	rsort.sort()
	restart=True
	while restart:
		restart=False
		for i in xrange(len(rsort)-1):
			...
			if prevnet.overlaps(nextnet) and prevnet.prefixlen-nextnet.prefixlen<0:
				#logger.debug("%s overlaps %s" % (prevnet, nextnet))
				rsort.pop(i+1)
				restart=True
				break
			if prevnet.prefixlen != nextnet.prefixlen: continue			
			prevsupernet = prevnet.Supernet()
			prevsuperset = set()
			if prevnet.network != prevsupernet.network: continue
			...
			if prevsuperset.difference(prevset).difference(nextset): continue
			#logger.debug("%s and %s are aregable" % (prevnet, nextnet))
			rsort.pop(i)
			rsort.pop(i)
			...
			break
	nets = []
	for n in rsort: nets.append(str(n))
	#logger.debug("Return %d agregated networks : %s" % (len(nets), str(nets)))	
	return(nets)
...
def hash_route(routes, addr):
	...
	if tm+MCAST_ANNOUNCE_TIME/10>MCAST_ANNOUNCE_TIME: t+=MCAST_ANNOUNCE_TIME
	t=t/MCAST_ANNOUNCE_TIME*MCAST_ANNOUNCE_TIME
	
	x = (hashlib.sha1(
		hashlib.sha512(str(routes)).hexdigest() +
		hashlib.sha256(addr).hexdigest() + 
#		hashlib.sha224(str(t)).hexdigest() +
		STATIC_KEY
	).hexdigest())
	#log.debug("Hash : %s\nNet : %s\nAddr : %s\nTime : %s\n Static : %s" % (x, routes, addr, t, STATIC_KEY))
	return(x)
...
def route_decode(jdata, addr):
	#logger.debug("Recieved from %s : %s" % (addr, data))
	data = json.loads(jdata)
	for net in list(data["networks"]):
		data["networks"].remove(net)
		data["networks"].append(str(net))
		
	if addr != data["localip"]: logger.warn("Pair %s annouunce route tagged as %s" % (addr, data["localip"]))
	elif hash_route(data["networks"], addr) != data["hash"]: logger.warn("Pair %s announce an invalid hash" % (addr))
	else:
		if not announcer.has_key(addr): announcer[addr] = {"announce_time":0, "last_announce":0}
		...
		oldnets = {}
		for r in routes: oldnets[r[1]] = r[2]
		for n in data["networks"]:
			net = dnet.addr(n)
			if net.bits < SECURITY: logger.warn("Dropping net %s when adding by security flag" % n)
			elif not net in oldnets.keys():
				logger.info("Adding route %s to %s" % (n, addr))
				try: iR.add(net, gateway)
				except: logger.exception("Error when adding route")
			elif oldnets[net] != gateway:
				logger.info("Replacing route %s (from %s to %s" % (n, oldnets[net], addr))
				try:
					iR.delete(net)
					iR.add(net, gateway)
				except: logger.excepton("Error when replacing route")
	...
	for addr, data in announcer.iteritems():
		if 3*data["announce_time"]+data["last_announce"]<int(time.time()) or RemoveAll:
			for r in get_lapis_route(get_routes(), addr):
				if r[1].bits < SECURITY: logger.warn("Dropping net %s when deleting by security flag" % r[1])
				else:
					logger.info("Garbaging route %s (to %s, but unsure)" % (r[1], r[2]))
					try: iR.delete(r[1]) # FIXME #BUG #TODO l'interface de dnet ne permet pas de limiter la supression a une interface /!\ tres dangereux
					except: logger.exception("Error when garbaging route")
...
if __name__ == "__main__":
	...
	#Enregistrement des action de garbage final
	atexit.register(garbage_collector, True)

	while not flag_halt:
		#Recreaction du socket s'il a ete vide
		if not sock: sock = create_sock()
	
		# Reception des annonces
		try: data, (addr, _) = sock.recvfrom(MCAST_MAX_MESSAGE_SIZE)
		except socket.timeout: pass
		except KeyboardInterrupt: flag_halt=True
		else:
			#logger.debug("Reciveing")
			try: route_decode(data, addr)
			except socket.error:
		...
		if (last_gc - time.time() + NET_GC_TIME) < 0:
			#logger.debug("Garbaging")
			last_gc = time.time()
			try: garbage_collector()
			except: 
		...
		if (last_calc - time.time() + NET_CALC_TIME) < 0:
			...
			last_calc = time.time()
			try: networks = get_network_to_announce()
			except:
		...
		if (last_announce - time.time() + MCAST_ANNOUNCE_TIME) < 0:
			...
			if networks:
				...
				try: route_announce(networks)
				except socket.error:
					sock = None
					logger.exception("Major error when Announcing")
					continue
			else: logger.debug("No networks, do not announce")
